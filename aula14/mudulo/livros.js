...
const liv = function (books) {
    ...
    livros.forEach(item => {
        item.books.forEach(itemBooks => {
            if(itemBooks.title.toLowerCase().indexOf(livro.toLowerCase()) > -1){
                livroContainer.push({titulo: itemBooks.title, descricao: itemBooks.subtitle, preco: itemBooks.price, foto: itemBooks.image})
                erro = false
            }
        })
    });

    jason.total = livroContainer.length
    jason.livros = livroContainer
